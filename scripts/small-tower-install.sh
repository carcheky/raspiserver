#!/bin/bash
# modified version with dinamic user name and absminitowerkit to temp folder
# https://github.com/geeekpi/absminitowerkit/blob/main/install.sh

# force run as: sudo ./scripts/small-tower-install.sh 
# or: sudo bash ./scripts/small-tower-install.sh
...
# test if /home/$USER/absminitowerkit exists, if not, clone
if [ ! -d /home/$USER/absminitowerkit ]; then
    git clone https://github.com/geeekpi/absminitowerkit /home/$USER/absminitowerkit
fi
...
# grant privilledges to user $USER.
...
# as to reboot now or not.
read -p "Do you want to reboot now? [y/n] " answer
if [ "$answer" != "${answer#[Yy]}" ] ;then
   sudo reboot
else
   log_action_msg "Please reboot Raspberry Pi to take effect." 
fi
