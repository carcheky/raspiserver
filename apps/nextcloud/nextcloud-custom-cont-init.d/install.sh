...
get_install_opts_for_apk() {
    flags=$(get_install_opts_for "apk")
    RETVAL="$flags"
}
...
# Install NordVPN for Alpine Linux
...
install_apk() {
    if check_cmd apk; then
        ...
        if check_cmd wget; then
            wget -qO - "${PUB_KEY}" | $SUDO tee /etc/apk/keys/nordvpn.asc > /dev/null
        elif check_cmd curl; then
            curl -s "${PUB_KEY}" | $SUDO tee /etc/apk/keys/nordvpn.asc > /dev/null
        else
            echo "Couldn't find wget or curl - one of them is needed to proceed with the installation"
            exit 1
        fi
        ...
        # Add the repository
        echo "https://${REPO_URL_RPM}/alpine/v3.12/main" | $SUDO tee -a /etc/apk/repositories

        # Install NordVPN
        $SUDO apk ${install_opts:-} update
        $SUDO apk ${install_opts:-} add nordvpn
        exit
    fi
}
